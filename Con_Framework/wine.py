...
df = pd.read_csv("Con_Framework/data/WineQT.csv")
...
target = "quality"
features = df.columns[(df.columns != target) & (df.columns != "Id")] #Todas las columnas menos la columna target y la columna id
...
X_train, X_test, y_train, y_test = train_test_split(df[features], df['quality'], test_size=0.3, random_state=42)
...
# Se puede ver en la grafica que el 95% de varianza se explica entre el componente 6 y 8. 
# Entones, con esa informacion, podemos reducir las caracteristicas a 7 y mantener el 95% de la informacion original.
...
knee_locator = KneeLocator(x = range(1, len(distances)+1), y = k_distances, curve="convex", direction="increasing")
...
# dandonos solo dos cluster mal distribuido con muchos datos clasificados como ruido.